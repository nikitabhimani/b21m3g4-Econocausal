...
def main():
    ...
    if df["customer_id"].duplicated().any():
        raise ValueError("Input data contains duplicate customer_id values.")
    ...
    if not all(len(values) == len(df) for values in (mu_0, mu_1, ite)):
        raise ValueError("The causal model returned an incomplete prediction set.")
    if not all(np.isfinite(values).all() for values in (mu_0, mu_1, ite)):
        raise ValueError("The causal model returned non-finite predictions.")
    ...
    if len(results_df) != len(df) or set(results_df["customer_id"]) != set(df["customer_id"]):
        raise ValueError("Prediction coverage does not match the input customer IDs.")
